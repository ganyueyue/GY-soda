...
- (UIView *)alertView {
    if (!_alertView) {
        _alertView = [[UIView alloc]initWithFrame:CGRectMake(0, SCREEN_HEIGHT - kPickerHeight, SCREEN_WIDTH, kPickerHeight)];
...
- (UIButton *)leftBtn {
    if (!_leftBtn) {
        ...
        if ([STAppEnvs shareInstance].isIphoneX && ([[UIApplication sharedApplication] statusBarOrientation] == UIInterfaceOrientationLandscapeRight || [[UIApplication sharedApplication] statusBarOrientation] == UIInterfaceOrientationLandscapeLeft)) {
            _leftBtn.frame = CGRectMake(35, 8, 60, 28);
        } else {
...
- (UIButton *)rightBtn {
    if (!_rightBtn) {
        ...
        if ([STAppEnvs shareInstance].isIphoneX && ([[UIApplication sharedApplication] statusBarOrientation] == UIInterfaceOrientationLandscapeRight || [[UIApplication sharedApplication] statusBarOrientation] == UIInterfaceOrientationLandscapeLeft)) {
            _rightBtn.frame = CGRectMake(self.alertView.frame.size.width - 75, 8, 60, 28);
        } else {
