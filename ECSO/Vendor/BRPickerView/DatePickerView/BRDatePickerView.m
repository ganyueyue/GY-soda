...
- (instancetype)initWithTitle:(NSString *)title
    ...
    if (self = [super init]) {
        ...
        if (minDate) {
            ...
            if (self.showType == BRDatePickerModeTime || self.showType == BRDatePickerModeCountDownTimer) {
                self.minLimitDate = [NSDate br_setHour:0 minute:0];
            } else if (self.showType == BRDatePickerModeHMS) {
                self.minLimitDate = [NSDate br_setHour:0 minute:0 second:0];
        ...
        if (maxDate) {
            ...
            if (self.showType == BRDatePickerModeTime || self.showType == BRDatePickerModeCountDownTimer) {
                ...
                self.maxLimitDate = [NSDate br_setHour:23 minute:59 second:59];
            } else if (self.showType == BRDatePickerModeMDHM) {
...
- (void)initDefaultDateArray {
    ...
    [self setupSecondArr:self.selectDate.br_year month:self.selectDate.br_month day:self.selectDate.br_day hour:self.selectDate.br_hour minute:self.selectDate.br_minute];
    ...
    _secondIndex = self.selectDate.br_second - ((_yearIndex == 0 && _monthIndex == 0 && _dayIndex == 0 && _hourIndex == 0 && _minuteIndex == 0) ? self.minLimitDate.br_second : 0);
...
- (void)setupSecondArr:(NSInteger)year month:(NSInteger)month day:(NSInteger)day hour:(NSInteger)hour minute:(NSInteger)minute {
    ...
    if (year == self.minLimitDate.br_year && month == self.minLimitDate.br_month && day == self.minLimitDate.br_day && hour == self.minLimitDate.br_hour && minute == self.minLimitDate.br_minute) {
        startSecond = self.minLimitDate.br_second;
    }
    if (year == self.maxLimitDate.br_year && month == self.maxLimitDate.br_month && day == self.maxLimitDate.br_day && hour == self.maxLimitDate.br_hour && minute == self.maxLimitDate.br_minute) {
        endSecond = self.maxLimitDate.br_second;
    }
...
- (void)scrollToSelectDate:(NSDate *)selectDate animated:(BOOL)animated {
    ...
    NSInteger secondIndex = selectDate.br_second - ((yearIndex == 0 && monthIndex == 0 && dayIndex == 0 && hourIndex == 0 && minuteIndex == 0) ? self.minLimitDate.br_second : 0);
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(nullable UIView *)view {
    ...
    if (!label) {
        ...
        label.font = [UIFont systemFontOfSize:15.0f * kScaleFit];
        // 字体自适应属性
...
- (NSDate *)getDidSelectedDate:(NSInteger)component row:(NSInteger)row {
    ...
    if (self.showType == BRDatePickerModeYMDHMS) {
        ...
        selectDateValue = [NSString stringWithFormat:@"%02ld:%02ld:%02ld", [self.hourArr[_hourIndex] integerValue], [self.minuteArr[_minuteIndex] integerValue], [self.secondArr[_secondIndex] integerValue]];
    }
