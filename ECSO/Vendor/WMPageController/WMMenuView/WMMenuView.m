...
- (void)btnAction:(UIButton *)sender{
    WMMenuItem *item = [self.scrollView viewWithTag:sender.tag - 1000];
...
- (void)addItems {
    ...
    for (int i = 0; i < self.titlesCount; i++) {
        ...
        if (self.selectfontName){
            item.selectedFont = [UIFont fontWithName:self.selectfontName size:item.selectedSize];
            if(self.fontName) {
                item.font = [UIFont fontWithName:self.fontName size:item.selectedSize];
                item.unselectedFont = [UIFont fontWithName:self.fontName size:item.selectedSize];
            }else{
                item.font = [UIFont systemFontOfSize:item.selectedSize];
                item.unselectedFont = [UIFont systemFontOfSize:item.selectedSize];
            }
        }else{
            if(self.fontName) {
                item.font = [UIFont fontWithName:self.fontName size:item.selectedSize];
